...
export function ApiGraph({ selected, onSelect, highlight, broken }: Props) {
  const graph = useMemo(() => apiGraph(), [])
  const width = 1120
  const height = 620
  const nodeW = 132
  const nodeH = 46
  const layerHeights = 78
  ...
  const positioned = graph.nodes.map((node) => {
    ...
    const y = 28 + node.layer * layerHeights
      ...
      >
        {graph.edges.map((edge) => {
          ...
          const active =
            ...
            selected === edge.from ||
            selected === edge.to
          const risk = broken.has(edge.from) || broken.has(edge.to)
          return (
            <path
